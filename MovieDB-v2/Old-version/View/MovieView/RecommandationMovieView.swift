...
class RecommandationMovieView: UIView {
    ...
    func getServerData(){
        APIClient.getMovieRecommandationId( id: GLOBAL_MOVIE_ID) { (response, error) in
    ...
    func setupUI(){
        ...
        collectionView.register(RecommandCollectionCell.self, forCellWithReuseIdentifier: "CELL_ID")
        ...
        let attr1: NSMutableAttributedString = getAttributedText(string: "Rcommandation ", font: UIFont(name: appFontBold, size: 18)!, color: .black, lineSpace: 5, alignment: .left)
...
extension RecommandationMovieView:  UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return result.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "CELL_ID", for: indexPath) as! RecommandCollectionCell
         cell.backgroundColor = .clear
        let index = result[indexPath.item]
        
        if index.posterPath != nil {
